...
export default function FileListItem2({
  ...
  return (
    <div
      ...
      className="w-100 mt-2 flex h-fit cursor-pointer flex-row rounded-md border border-border-light bg-surface-secondary p-4 text-text-primary transition duration-300 ease-in-out hover:bg-surface-hover"
    >
      <div className="flex w-10/12 flex-col justify-around md:flex-row">
        ...
        <div className="flex w-fit flex-row flex-wrap text-text-secondary md:w-3/5">
          {attachedVectorStores.map((vectorStore, index) => {
            if (index === 4) {
              return (
                <span
                  key={index}
                  className="ml-2 mt-1 flex flex-row items-center rounded-full bg-surface-tertiary px-2 text-xs text-text-secondary"
                >
            ...
            return (
              <span
                key={index}
                className="ml-2 mt-1 content-center rounded-full bg-green-100 px-2 text-xs text-green-700 dark:bg-green-900/30 dark:text-green-400"
              >
                {vectorStore.name}
              </span>
            );
      ...
      <div className="mr-0 flex w-2/12 flex-col items-center justify-evenly sm:mr-4 md:flex-row">
        <Button className="w-min content-center bg-transparent text-text-secondary hover:bg-surface-hover">
          <DotsIcon className="text-text-secondary" />
        </Button>
        <Button
          className="w-min bg-transparent text-text-secondary hover:bg-surface-hover"
