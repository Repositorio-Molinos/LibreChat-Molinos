...
export default function FileListItem({ file, deleteFile, width = '400px' }: FileListItemProps) {
  return (
    <div className="w-100 my-3 mr-2 flex cursor-pointer flex-row rounded-md border border-border-light bg-surface-secondary p-4 text-text-primary transition duration-300 ease-in-out hover:bg-surface-hover">
      <div className="flex w-1/2 flex-col justify-around align-middle">
        <strong>{file.filename}</strong>
        <p className="text-sm text-text-secondary">{file.object}</p>
      </div>
      ...
      <div className="flex w-1/6 justify-around">
        <Button
          className="my-0 ml-3 bg-transparent p-0 text-text-secondary hover:bg-surface-hover"
